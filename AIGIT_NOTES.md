...
### Step 11 — DONE
Mark-and-sweep gc. **Mark**: for every ref under `refs/heads/*` (this repo has no detached-HEAD checkout and no stash/reflog, so branch refs + the index are the complete root set), walks first-parent commit history iteratively (extra merge parents walked recursively -- bounded by merge count, not history length) via `Core::loadCommit`; each commit's tree via `Core::loadTree`, recursively; each blob via `ObjectStore::retrieve`, and if its content is a chunk manifest (same JSON-shape check `StorageManager::restoreFile` already uses), its chunk hashes too. Also marks every blob the staging index (`.aigit/index`) points at, so staged-but-uncommitted content survives. **Sweep**: `ObjectStore::walkAll()` over every on-disk object; anything not in the marked set is deleted via a new `ObjectStore::remove()` + `MetadataDB::removeObject()`.

Safety: if marking hits an unreadable commit/tree/blob anywhere, or the disk walk finds *any* corrupt object, gc refuses to delete anything at all and exits non-zero pointing at `fsck` -- a broken link partway through history could otherwise hide something that's actually still reachable, so the safe default is "do nothing," not "guess." `ObjectStore::remove()` is a new method but is only ever called from gc's sweep, after both checks pass.

Wired to a new `ai-git gc` command. Test added: `gc_test.cpp` -- two commits (so the first commit's blob/tree/commit are only reachable through history, not the current tree), a staged-but-uncommitted file, and a hand-injected orphan object with nothing pointing at it. Asserts gc removes exactly the orphan (on disk and from MetadataDB) and keeps everything else, including the staged blob.
